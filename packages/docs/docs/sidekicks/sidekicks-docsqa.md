...
Once a Source has been added to a Corpus, Fixie takes care of fetching the documents
from that Source (which might involve crawling web pages and following links).
Documents are then processed in various ways -- for example, HTML contents are
converted to Markdown, and raw text is extracted from PDF files. Each document
is then converted into a set of **Chunks**. Each Chunk represents a small piece
of the document contents -- usually a few kilobytes in size -- that the Large Language
Model can process independently of the rest of the document.

Fixie uses the LLM to generate a set of **vector embeddings** for each Chunk.
...
query. All of this is just a fancy way of giving the LLM access to a potentially large
number of documents by splitting the docs up into smaller pieces that the LLM is
able to process.
