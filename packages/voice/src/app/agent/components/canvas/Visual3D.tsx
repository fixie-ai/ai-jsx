...
import { AVAILABLE_COLOR_PALETTES, ColorPalette, ColorPaletteType, COLOR_PALETTE } from '../visualizers/palettes';
...
const getVisualizerComponent = (mode: ApplicationMode, visual: string, palette: ColorPaletteType) => {
  switch (mode) {
    case APPLICATION_MODE.WAVE_FORM:
      return <WaveformVisual visual={visual} palette={palette} />;
    case APPLICATION_MODE.NOISE:
      return visual === 'particleSwarm' ? <ParticleNoiseVisual /> : <NoiseVisual visual={visual} palette={palette} />;
...
const Visual3DCanvas = ({ mode }: Visual3DCanvasProps) => {
  ...
  const { visualizer } = useControls({
    visualizer: {
      value: visualizerParam && AVAILABLE_VISUALS.includes(visualizerParam) ? visualizerParam : AVAILABLE_VISUALS[1],
  ...
  const backgroundColor = colorBackground ? ColorPalette.getPalette(palette).calcBackgroundColor(0) : '#010204';
  return (
    <Canvas
      camera={{
        fov: 45,
