...
const SphereVisual = ({ coordinateMapper, palette = COLOR_PALETTE.THREE_COOL_TO_WARM }: VisualProps) => {
  const {
    radius,
    nPoints,
    cubeSideLength,
    ...
    'Visual - Sphere': folder(
      {
        ...
        nPoints: { value: 2000, min: 100, max: 2000, step: 25 },
        cubeSideLength: {
          value: 0.01,
          min: 0.01,
          max: 0.5,
  ...
  return (
    <>
      <BaseSphere
        ...
        palette={palette}
      />
      <Ground position={new Vector3(0, 0, -radius * (1 + 0.25 * coordinateMapper.amplitude))} />
    </>
  );
};
