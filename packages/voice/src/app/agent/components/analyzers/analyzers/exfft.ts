...
export default class ExternalFFTAnalyzer implements AnalyzerInputControl {
  ...
  constructor(localAnalyzer: AnalyserNode, remoteAnalyzer: AnalyserNode) {
    this._analyzer = localAnalyzer;
    ...
    // this should probably be set externally +++++
  ...
  private _freqToBin(freq: number, round: boolean = true): number {
    const max = this._analyzer.frequencyBinCount - 1,
      bin = (round ? Math.round : Math.floor)((freq * this._analyzer.fftSize) / this._audioCtx.sampleRate);
