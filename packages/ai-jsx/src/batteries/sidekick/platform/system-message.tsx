...
export function SidekickSystemMessage({
  ...
  const dateTimeSystemMessage = (
    <SystemMessage>
      ...
      {timeZone}, which has an offset from UTC of {timeZoneOffset}. When giving
      dates, use the user's time zone. If the user does not specify a date or
      time, assume it is intended either now or in the near future.
    </SystemMessage>
  )

  const responseFramingSystemMessage = (
    <SystemMessage>
      Do not say "according to the documents I have been given"; just answer as
      if you know the answer. When you see timestamps in your source data,
      format them in a nice, human-readable way. Speak politely but casually.
