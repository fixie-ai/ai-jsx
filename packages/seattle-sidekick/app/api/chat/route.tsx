...
export async function* UseTools(props: UseToolsProps, { render }: AI.RenderContext) {
  ...
  if (props.fallback) {
    messages.push(
      <SystemMessage>
        Here{"'"}s the fallback strategy/message if something failed: {props.fallback}
      </SystemMessage>
    );
  }
  ...
  const tools: Record<string, Tool> = {
    ...
    getLatLongOfLocation: {
      ...
      parameters: {
        location: {
          description:
            'The location to get the lat/long coordinates of, e.g. "Seattle, WA" or "123 My Street, Bellevue WA"',
          type: 'string',
          required: true,
        },
      },
    ...
    getDirections: {
      description:
        'Get directions between two locations via a variety of ways (walking, driving, public transit, etc.)',
      parameters: {
        origin: {
          description:
            'The starting location, e.g. "South Lake Union, Seattle, WA" or "123 My Street, Bellevue WA", or a Google Maps place ID, or lat/long coords. If you give a location name, you need to include the city and state. Do not just give the name of a neighborhood.',
          type: 'string',
          required: true,
        },
        destination: {
          description:
            'The ending location, e.g. "Fremont, Seattle, WA" or "123 My Street, Bellevue WA", or a Google Maps place ID, or lat/long coords. If you give a location name, you need to include the city and state. Do not just give the name of a neighborhood.',
          type: 'string',
          required: true,
        },
      },
    ...
    <OpenAI chatModel="gpt-4">
      <ChatCompletion>
        <SystemMessage>
          ...
          You help users with plan activities in Seattle. You can look for locations and find directions. If a user asks
          for anything not related to that, tell them you can{"'"}t help. If the user asks for location information and
          directions, you will be given live API calls in subsequent systems messages. You should respond to the user
          {"'"}s request using the results of those API calls. If those API calls errored out, tell the user there was
          an error making the request. Do not tell them you will try again.
          {/* This is not respected. */}
          Do not attempt to answer using your latent knowledge. Respond concisely, using markdown formatting to make
          your response more readable and structured. Do not say {"'"}now retrieving data{"'"} or {"'"}I{"'"}ll query
          that for you{"'"} or anything like that. All the queries are already done. Do not say {"'"}I can help with
          that{"'"} if you are not actually giving the full answer.
        </SystemMessage>
        <NaturalLanguageRouter query={latestMessage.content!}>
          <Route when="to respond to the user's request, it would be helpful to get directions">
            <SystemMessage>
              API results of directions the user asked for:{' '}
              <UseTools
                tools={tools}
                fallback="Tell the user there was an error making the request."
                messages={messages}
              />
            </SystemMessage>
          </Route>
          <Route when="to respond to the user's request, it would be helpful search for locations">
            <SystemMessage>
              Do not use your own knowledge about places. Instead, use the results of this live API call:{' '}
              <UseTools
                tools={tools}
                fallback="Tell the user there was an error making the request."
                messages={messages}
