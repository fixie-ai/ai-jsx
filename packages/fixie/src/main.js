...
function showResult(result, raw) {
    if (raw) {
...
function registerDeployCommand(command) {
    command
        .command('deploy [path]')
        .description('Deploy an agent')
        .option('-e, --env <key=value>', 'Environment variables to set for this deployment. Variables in a .env file take precedence over those on the command line.', (v, m) => {
...
function registerServeCommand(command) {
    command
        .command('serve [path]')
        .description('Run an agent locally')
        .option('-p, --port <number>', 'Port to run the agent on', '8181')
        .option('-e, --env <key=value>', 'Environment variables to set for this agent. Variables in a .env file take precedence over those on the command line.', (v, m) => {
        const [key, value] = v.split('=');
        return {
            ...m,
            // This condition is necessary; the types are wrong.
...
program
    ...
    .action(catchErrors(async (options) => {
...
corpus
    ...
    .addOption(new Option('-o, --owner <ownerType>', 'Type of corpora to list.').choices(['user', 'org', 'public', 'all']))
    .action(catchErrors(async ({ owner }) => {
    const client = await AuthenticateOrLogIn({ apiUrl: program.opts().url });
    let ownerType = 'OWNER_ALL';
    if (owner === 'user') {
...
corpus
    ...
    .action(catchErrors(async (corpusId) => {
    const client = await AuthenticateOrLogIn({ apiUrl: program.opts().url });
...
corpus
    ...
    .action(catchErrors(async (name, description) => {
    const client = await AuthenticateOrLogIn({ apiUrl: program.opts().url });
...
corpus
    ...
    .action(catchErrors(async (corpusId, query) => {
    const client = await AuthenticateOrLogIn({ apiUrl: program.opts().url });
...
source
    ...
    .action(catchErrors(async (corpusId, startUrls, { maxDocuments, maxDepth, includePatterns, excludePatterns, description, }) => {
    if (!includePatterns) {
        term.yellow('Warning: ')('No --include-patterns specfied. This is equivalent to only crawling the URLs specified as startUrls.\n');
        term.yellow('Warning: ')('Use ').red("--include-patterns '*'")(' if you want to allow all URLs in the crawl.\n');
    }
    const client = await AuthenticateOrLogIn({ apiUrl: program.opts().url });
    const result = await client.addCorpusSource(corpusId, startUrls, includePatterns, excludePatterns, maxDocuments, maxDepth, description);
    showResult(result, program.opts().raw);
}));
source
    .command('upload <corpusId> <mimeType> <filenames...>')
    .description('Upload local files to a corpus.')
    .action(catchErrors(async (corpusId, mimeType, filenames) => {
    const client = await AuthenticateOrLogIn({ apiUrl: program.opts().url });
...
source
    ...
    .action(catchErrors(async (corpusId, sourceId) => {
    const client = await AuthenticateOrLogIn({ apiUrl: program.opts().url });
...
source
    ...
    .option('--force', 'By default, this command will fail if you try to refresh a source that currently has a job running. If you want to refresh the source regardless, pass this flag.')
    .action(catchErrors(async (corpusId, sourceId, { force }) => {
    const client = await AuthenticateOrLogIn({ apiUrl: program.opts().url });
...
source
    ...
    .option('--force', 'By default, this command will fail if you try to clear a source that currently has a job running. If you want to clear the source regardless, pass this flag.')
    .action(catchErrors(async (corpusId, sourceId, { force }) => {
    const client = await AuthenticateOrLogIn({ apiUrl: program.opts().url });
...
job
    ...
    .action(catchErrors(async (corpusId, sourceId, jobId) => {
    const client = await AuthenticateOrLogIn({ apiUrl: program.opts().url });
...
doc
    ...
    .action(catchErrors(async (corpusId, sourceId, docId) => {
    const client = await AuthenticateOrLogIn({ apiUrl: program.opts().url });
...
agent
    ...
    .action(catchErrors(async (agentId) => {
    const client = await AuthenticateOrLogIn({ apiUrl: program.opts().url });
...
agent
    ...
    .action(catchErrors(async (agentHandle) => {
    const client = await AuthenticateOrLogIn({ apiUrl: program.opts().url });
...
agent
    ...
    .action(catchErrors(async (agentHandle, agentName, agentDescription, agentMoreInfoUrl) => {
    const client = await AuthenticateOrLogIn({ apiUrl: program.opts().url });
...
revision
    ...
    .action(catchErrors(async (agentId, revisionId) => {
    const client = await AuthenticateOrLogIn({ apiUrl: program.opts().url });
    const agent = await FixieAgent.GetAgent(client, agentId);
